...
// ---------------- Moving Average Filter (GPIO2 Channel 1) ----------------
#define GPIO2_MA_LEN  16   // moving average window length

static volatile u32 gpio2_ma_buf[GPIO2_MA_LEN] = {0};
static volatile u32 gpio2_ma_sum = 0;
static volatile u32 gpio2_ma_idx = 0;
static volatile u32 gpio2_ma_count = 0;     // number of valid samples collected (<= GPIO2_MA_LEN)
static volatile u32 gpio2_in_avg = 0;       // averaged period
// ------------------------------------------------------------------------
 ...
 * We apply a moving average filter to period, then compute RPM.
 */
void ExternalIsr(void *CallbackRef)
{
    ...
    // ---------------- Moving average on period ----------------
    if(gpio2_ma_count >= GPIO2_MA_LEN) {
        gpio2_ma_sum -= gpio2_ma_buf[gpio2_ma_idx];
    } else {
        gpio2_ma_count++;
    }
    ...
    gpio2_in_avg = gpio2_ma_sum / gpio2_ma_count;
    // ----------------------------------------------------------

    // Compute RPM using AVERAGED period
...
int main()
{
    ...
    while(1) {
        ...
        if(c=='s') { xil_printf("S:"); spd=(u16)GetNum(); Upd(); }
            ...
            xil_printf("G2_period:%u avg:%u  RPM:%u  G3:%d  IntCnt:%u\r\n",
                       gpio2_in, gpio2_in_avg, current_rpm, gpio3_in, gpio2_count);
